...
export const useChessStore = defineStore('chess', () => {
  ...
  const saveCompletedGame = async (durationSeconds: number) => {
    ...
    if (!identity || identity.type === 'representative' || !identity.member_id) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/roi/v1/games`, {
        method: 'POST',
        headers: {
      ...
      console.warn("Échec de l'envoi immédiat, mise en file d'attente hors ligne:", error);
      queuePendingGame(gameData);
    }
  };
  ...
  const queuePendingGame = (game: PendingGame) => {
    ...
    pending.push(game);
    localStorage.setItem('dame_pending_games', JSON.stringify(pending));
  };
  ...
  const syncPendingGames = async () => {
    ...
    localStorage.setItem('dame_pending_games', JSON.stringify(remaining));
