...
vi.mock('@/components/shared/QcmViewer.vue', () => ({
  default: {
    name: 'QcmViewer',
    props: ['question', 'choix', 'bonneReponse', 'shapes', 'fen'],
    emits: ['success'],
    template: `
      <div class="mock-qcm-viewer">
        <span class="mock-question">{{ question }}</span>
...
describe('Type100Commandements.vue', () => {
    ...
    const wrapper = mount(Type100Commandements, {
      ...
      props: {
        ...
        config: {
          ...
          reponses: ["Contrôler le centre", "Avancer la Dame", "Pions des bords"],
          bonne_reponse: 0
        }
      }
    });

    expect(wrapper.find('.mock-question').text()).toBe("Quel est le premier réflexe ?");
    expect(wrapper.find('.qcm-badge').exists()).toBe(false);
  ...
  test('handles series of QCMs correctly (qcms array)', async () => {
    ...
    // QCM 1
    expect(wrapper.find('.qcm-badge').text()).toContain('Question 1 / 2');
    expect(wrapper.find('.mock-question').text()).toBe("Question 1");
    ...
    // QCM 2
    expect(wrapper.find('.qcm-badge').text()).toContain('Question 2 / 2');
    expect(wrapper.find('.mock-question').text()).toBe("Question 2");
