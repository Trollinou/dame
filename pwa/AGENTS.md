...
- **Appels HTTP & Rafraîchissement Transparent JWT (`safeFetch` & `fetchWpCollection`)** :
  - Tous les appels HTTP vers les APIs du serveur (WordPress REST API, DAME, ROI) doivent **impérativement s'appuyer sur la fonction utilitaire `safeFetch`** (`src/utils/safeFetch.ts`) ou la fonction d'abstraction de collection paginée **`fetchWpCollection`** (`src/utils/wpApi.ts`) au lieu du `fetch` natif.
  - **Récupération des Collections WP REST Paginées (`fetchWpCollection`)** : Pour récupérer des collections complètes (membres, contacts, messages, saisons, etc.), utiliser la fonction utilitaire `fetchWpCollection<T>(path)` qui gère automatiquement l'injection du jeton JWT (`Authorization: Bearer`), le parcours de l'ensemble des pages (`X-WP-TotalPages`), la vérification stricte du statut HTTP (`res.ok`), et la fusion des résultats.
  - **Gestion automatique du HTTP 401** : `safeFetch` (et par extension `fetchWpCollection`) intercepte automatiquement le statut HTTP 401, sollicite le rafraîchissement transparent du jeton JWT via `authStore.tryRefreshToken()`, et réessaie la requête initiale avec le nouveau jeton avant d'envisager toute déconnexion.
  - **Interdiction de déconnexion directe** : Il est strictly interdit de faire un `authStore.logout()` immédiat sur un statut 401 dans les requêtes de store sans être passé par la tentative de rafraîchissement de `safeFetch`.
