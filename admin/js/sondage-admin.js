jQuery(document).ready(function ($) {
	...
	$('#add-sondage-date').on('click', function () {
		...
		const newDateGroup =
			...
			`</h4>
                <p>
                    <label for="sondage_date_` +
			dateIndex +
			`">Date:</label>
                    <input type="date" id="sondage_date_` +
			dateIndex +
			`" name="_dame_sondage_data[` +
			dateIndex +
			`][date]" value="" class="sondage-date-input">
	...
	$('#sondage-dates-wrapper').on(
		...
		function () {
			...
			// Re-index h4 titles
			$('#sondage-dates-wrapper .sondage-date-group').each(function (
				index
			) {
				$(this)
					.find('h4')
	...
	$('#sondage-dates-wrapper').on(
		...
		function () {
            ...
            <div class="sondage-time-slot-group">
                ...
                <input type="time" name="_dame_sondage_data[` +
				dateIndex +
				`][time_slots][` +
				timeIndex +
				`][start]" value="` +
