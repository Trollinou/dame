...
function dame_add_message_row_actions( $actions, $post ) {
    if ( 'dame_message' === $post->post_type && current_user_can( 'publish_posts' ) ) {
        // Duplicate link.
...
/**
 * Handles the duplication of a message as a new post.
 *
 * Triggers on the 'admin_action_dame_duplicate_as_post' hook.
 */
function dame_handle_duplicate_as_post_action() {
    // Check for required GET parameters.
    if ( ! isset( $_GET['action'], $_GET['post'], $_GET['_wpnonce'] ) ) {
        return;
    }
    ...
    // Verify the action and post type.
    if ( 'dame_duplicate_as_post' !== $_GET['action'] || 'dame_message' !== get_post_type( $_GET['post'] ) ) {
        return;
    }

    // Verify the nonce for security.
    if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'dame_duplicate_as_post_' . $_GET['post'] ) ) {
        wp_die( 'Security check failed.' );
    }
    ...
    $new_post = array(
        ...
        'post_content' => $post->post_content,
        'post_status'  => 'draft',
        'post_type'    => 'post', // Create a standard post
        'post_author'  => $new_post_author->ID,
    );
    ...
    if ( $new_post_id && ! is_wp_error( $new_post_id ) ) {
        // Redirect to the edit screen of the new draft post.
        wp_safe_redirect( admin_url( 'post.php?action=edit&post=' . $new_post_id ) );
        exit;
    } else {
        // Redirect back to the message list with an error.
        wp_safe_redirect( admin_url( 'edit.php?post_type=dame_message&dame_error=duplicate_failed' ) );
        exit;
    }
}
add_action( 'admin_action_dame_duplicate_as_post', 'dame_handle_duplicate_as_post_action' );
