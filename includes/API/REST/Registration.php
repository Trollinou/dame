...
class Registration {
	...
	public function init(): void {
		...
		add_action( 'init', array( $this, 'handle_token_verification' ) );
		add_filter( 'wp_authenticate_user', array( $this, 'block_unverified_login' ), 10, 2 );
	}
	...
	public function handle_token_verification(): void {
		...
		if ( empty( $users ) ) {
			wp_die( esc_html__( 'Jeton de vérification invalide.', 'dame' ) );
		}
		...
		if ( time() > $expiration ) {
			wp_die( esc_html__( 'Le jeton de vérification a expiré.', 'dame' ) );
		}
	...
	public function block_unverified_login( $user, $password ): WP_User|WP_Error {
		if ( $user instanceof WP_User ) {
			...
			$allowed_roles = array( 'membre', 'staff', 'entraineur', 'editor', 'administrator' );
