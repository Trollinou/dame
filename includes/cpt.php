    ...
    if ( is_singular( 'dame_agenda' ) && in_the_loop() && is_main_query() ) {
        ...
        if ( ! empty( $location ) ) {
            ...
            if ( ! empty( $latitude ) && ! empty( $longitude ) ) {
                ...
                $details_html .= '<a href="https://www.google.com/maps/dir/?api=1&destination=' . esc_attr( $latitude ) . ',' . esc_attr( $longitude ) . '" target="_blank" class="nav-button">📱 ' . __( 'Calculer l\'itinéraire', 'dame' ) . '</a>';
                $details_html .= '<a href="geo:' . esc_attr( $latitude ) . ',' . esc_attr( $longitude ) . '?q=' . esc_attr( $latitude ) . ',' . esc_attr( $longitude ) . '(' . esc_attr( $location ) . ')" class="nav-button">🧭 ' . __( 'Ouvrir dans le GPS', 'dame' ) . '</a>';
                $details_html .= '</div>';
                $details_html .= '</div>';
            }
        }
