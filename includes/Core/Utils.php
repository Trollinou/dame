<?php
/**
 * Utility functions for the DAME plugin.
 *
 * @package DAME
 */
...
class Utils {
	 ...
	 */
	public static function format_lastname( $name ) {
	 ...
	 */
	public static function format_firstname( $name ) {
	 ...
	 */
	public static function get_contrast_color( $hexcolor ) {
	 ...
	 */
	public static function get_text_color_based_on_bg( $hex_color ) {
	 ...
	 */
	public static function lighten_color( $hex_color, $percentage ) {
		$rgb = self::hex_to_rgb( $hex_color );
	 ...
	 */
	public static function get_adherent_age_category_key( $birth_date_str, $gender = 'Masculin' ) {
		$category = self::get_adherent_age_category( $birth_date_str, $gender );
	...
	public static function get_message_recipients( $message_id ) {
		...
		if ( 'group' === $selection_method ) {
			...
			if ( ! empty( $seasons ) || ! empty( $saisonnier_groups ) || ! empty( $permanent_groups ) ) {
				$query_args = array(
					'post_type'      => 'adherent',
					'posts_per_page' => -1,
					'fields'         => 'ids',
				...
				for ( $i = 1; $i <= 2; $i++ ) {
					...
					if ( ! empty( $rep_email ) && is_email( $rep_email ) && '1' !== $rep_refuses_comms ) {
						if ( ! array_key_exists( $rep_email, $recipients ) ) {
							...
							$recipients[ $rep_email ] = trim( self::format_firstname( $rep_first_name ) . ' ' . self::format_lastname( $rep_last_name ) );
						}
					}
				}
