 ...
 *
 * Handles the "Assignation des comptes" page.
 */
class UserAssignment {
	...
	public function handle_assignment() {
		...
		if ( ! $member_id || ! get_post( $member_id ) ) {
			// This case is unlikely if the form is generated correctly, but handle it anyway.
			// Ideally we would redirect with an error message.
			return;
		}

		if ( ! $user_id ) {
			wp_safe_redirect( admin_url( 'edit.php?post_type=adherent&page=dame-user-assignment&message=missing_user' ) );
			exit;
		}
		...
		if ( ! $user ) {
			wp_safe_redirect( admin_url( 'edit.php?post_type=adherent&page=dame-user-assignment&message=invalid_user' ) );
			exit;
		}
		...
		if ( ! in_array( $role, array_keys( $wp_roles->get_names() ), true ) ) {
			wp_safe_redirect( admin_url( 'edit.php?post_type=adherent&page=dame-user-assignment&message=invalid_role' ) );
			exit;
		}
		...
		wp_safe_redirect( admin_url( 'edit.php?post_type=adherent&page=dame-user-assignment&message=success' ) );
		exit;
	}
	...
	public function display_notices() {
		if ( ! isset( $_GET['page'] ) || 'dame-user-assignment' !== $_GET['page'] ) {
			return;
		}
