document.addEventListener( 'DOMContentLoaded', function () {
	...
	const departmentSelectForRegion =
		document.getElementById( 'dame_department' );
	...
	if (
		...
		departmentSelectForRegion.addEventListener( 'change', function () {
			...
			const regionCode =
				dame_admin_data.department_region_mapping[ departmentCode ];
			if ( regionCode ) {
	...
	if ( categoryFilter && typeof dame_mailing_data !== 'undefined' ) {
		...
		const checkboxes = categoryFilter.querySelectorAll(
			'input[type="checkbox"]'
		);
	...
	if ( document.body.classList.contains( 'post-type-dame_agenda' ) ) {
		...
		if ( postForm ) {
			postForm.addEventListener( 'submit', function ( e ) {
				const startDate =
					document.getElementById( 'dame_start_date' ).value;
				const endDate =
					document.getElementById( 'dame_end_date' ).value;
