jQuery( document ).ready( function ( $ ) {
	...
	function fetchAndRenderCalendar() {
		...
		const startDayOfWeek =
			( firstDayOfMonth.getDay() - dame_agenda_ajax.start_of_week + 7 ) %
			7;
			...
			weekCells.each( function () {
				...
				if ( ponctuelContainer.length > 0 ) {
					...
					dayCell.find( '.dame-event-duree' ).each( function () {
						const eventEl = $( this );
						const eventBottom =
							eventEl.position().top +
							eventEl.outerHeight( true );
